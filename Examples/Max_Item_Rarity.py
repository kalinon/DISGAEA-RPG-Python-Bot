...
while True:
    ...
    a.etna_resort_refine_item(item['id'])
    a.upgrade_items(items=[item])
    a.player_items(True)
    prinny_steel = a.pd.get_item_by_m_item_id(Items.Prinny_Steel.value)['num']
    a.log(f"Equipment rarity increased. Current rarity: {item['rarity_value']}. Current prinny steel: {prinny_steel}")
